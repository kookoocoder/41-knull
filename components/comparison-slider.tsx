...
import { Download, RotateCcw, Sparkles, Share2, Copy, Check, Wand2 } from "lucide-react"
...
export function ComparisonSlider({ originalImage, restoredImage, fileName, onReset, onEdit, isMobile }: ComparisonSliderProps) {
  ...
  const handleDownload = () => {
    ...
    toast({
      ...
      description: "Your restored image is being downloaded.",
    })
  }
  ...
  const handleNativeShare = async () => {
    if ('share' in navigator) {
      try {
        ...
        const file = new File([blob], `restored_${fileName}`, { type: blob.type })

        await navigator.share({
          title: "Restored Image - RestoreAI",
          text: "Check out my restored image from RestoreAI!",
          files: [file],
        })

        toast({
          title: "Shared successfully",
          description: "Your restored image has been shared.",
        })
      } catch (error) {
  ...
  const handleCopyLink = async () => {
    try {
      ...
      if (navigator.clipboard && window.ClipboardItem) {
        ...
        toast({
          title: "Image copied",
          description: "The restored image has been copied to your clipboard.",
        })
      } else {
        // Fallback: copy the data URL as text
        await navigator.clipboard.writeText(restoredImage)
  ...
  const handleSaveAs = () => {
    ...
    const link = document.createElement("a")
    link.href = restoredImage
    link.download = `restored_${Date.now()}_${fileName}`
    ...
    toast({
      ...
      description: "Choose where to save your restored image.",
    })
  }
  ...
  return (
    ...
    >
      <div className="text-center space-y-3 md:space-y-4">
        ...
        >
          {isMobile ? "Slide to compare" : "Drag the slider to compare"} your original and restored images
        </motion.p>
      </div>
        ...
        <CardContent className="p-3 md:p-6 space-y-4 md:space-y-6">
          <div className="flex flex-wrap items-center justify-between gap-2 md:gap-3">
            ...
            <div className="flex gap-1 md:gap-2">
              {onEdit && (
                <Button onClick={() => onEdit(restoredImage)} size={isMobile ? "sm" : "default"} className="text-xs md:text-sm">
          ...
          {isMobile && (
            <div className="flex gap-2 pt-2">
              {onEdit && (
                <Button onClick={() => onEdit(restoredImage)} variant="outline" className="flex-1 text-xs">
